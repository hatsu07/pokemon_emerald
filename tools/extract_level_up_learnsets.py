...
DEFAULT_LEARNSET_DATA_END = 0x002F5CA4
    ...
    *,
    ...
    if not data_start <= offset < data_end:
        raise ValueError(
            ...
            f"0x{data_start:X}-0x{data_end:X}"
        )
    if offset % 2:
...
def render_data_include(
    ...
    *,
    ...
    if previous_end is None:
        raise ValueError("no learnsets were extracted")
    if previous_end > data_end:
        raise ValueError(
            f"learnset data exceeds configured end: "
            f"0x{previous_end:X} > 0x{data_end:X}"
        )

    if previous_end < data_end:
        tail = rom[previous_end:data_end]
        if tail == b"\x00\x00":
            lines.append("    .2byte 0x0000")
        else:
            lines.append(
                f'    .incbin "baserom.gba", 0x{previous_end:X}, '
                f'0x{data_end - previous_end:X}'
            )
...
def verify_extraction(
    ...
    *,
    ...
    for learnset in sorted(learnsets.values(), key=lambda item: item.offset):
        if learnset.offset < cursor:
            raise ValueError("overlapping learnsets detected during verification")

        rebuilt_data += rom[cursor:learnset.offset]
        encoded = encode_learnset(learnset)
        if learnset.end_offset > learnset_data_end:
            raise ValueError(
                f"learnset 0x{learnset.address:08X} falls outside data region"
            )
        rebuilt_data += encoded
        cursor = learnset.end_offset

    rebuilt_data += rom[cursor:learnset_data_end]

    original_data = rom[data_start:learnset_data_end]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--learnset-data-end",
        type=parse_int,
        default=DEFAULT_LEARNSET_DATA_END,
        help="end of the level-up learnset data as a ROM file offset",
    )
    parser.add_argument(
        "--pointer-table",
        type=parse_int,
...
def main() -> int:
    ...
    if not (
        ...
        raise SystemExit(
            ...
            f"learnsets 0x{args.data_start:X}-0x{args.learnset_data_end:X}, "
            f"pointers 0x{args.pointer_table:X}-0x{args.data_end:X}"
        )
    ...
    for pointer in sorted(set(pointers)):
        learnsets[pointer] = decode_learnset(
            ...
            data_end=args.learnset_data_end,
    ...
    if not args.no_verify:
        verify_extraction(
            rom,
            data_start=args.data_start,
            learnset_data_end=args.learnset_data_end,
    ...
    print(
        f"learnset data: "
        f"0x{args.data_start:08X}-0x{args.learnset_data_end:08X}"
    )
    print(
