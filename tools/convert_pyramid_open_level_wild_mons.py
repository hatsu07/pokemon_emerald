...
SOURCE_URL = (
    ...
    "battle_pyramid_open_level_wild_mons.h"
)

OUTPUT_PATH = Path(
    "data/battle_frontier/"
    "battle_pyramid_open_level_wild_mons.inc"
)

ROUND_PATTERN = re.compile(
    r"static const struct PyramidWildMon "
    r"sOpenLevelWildMons_Round(\d+)\[\]\s*=\s*"
    r"\{(.*?)\n\};",
    re.DOTALL,
)
...
ENTRY_PATTERN = re.compile(
    ...
    r"\.lvl\s*=\s*(\d+)\s*,\s*"
    r"\.abilityNum\s*=\s*([A-Z0-9_]+|\d+)\s*,\s*"
    r"\.moves\s*=\s*\{\s*"
    r"(MOVE_[A-Z0-9_]+)\s*,\s*"
    r"(MOVE_[A-Z0-9_]+)\s*,\s*"
    r"(MOVE_[A-Z0-9_]+)\s*,\s*"
...
def convert(text: str) -> str:
    ...
    for round_number_text, body in rounds:
        ...
        output.append(
            f"sOpenLevelWildMons_Round{round_number}::"
        )
        ...
        for (
            ...
            output.append(
                "\tpyramidwildmon "
                f"{species}, {level_offset}, "
                f"{convert_ability(ability)}, "
