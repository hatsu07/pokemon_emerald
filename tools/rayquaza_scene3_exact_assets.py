#!/usr/bin/env python3
# Exact repacker for Rayquaza Scene 3.
...
def encode_4bpp(path: Path, pixel_bytes: int) -> bytes:
    ...
    if width % 8 or height % 8:
        raise ValueError(f"{path}: dimensions must be tile aligned")
        ...
        for tx in range(0, width, 8):
            ...
            for y in range(8):
                ...
                for x in range(0, 8, 2):
                    ...
                    hi = pixels[base + x + 1]
                    if lo >= 16 or hi >= 16:
                        raise ValueError(f"{path}: 4bpp pixel index exceeds 15")
                    out.append(lo | (hi << 4))
            tile += 1
    ...
    for lineno, source in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        for token in m.group(1).split(","):
            ...
            if not re.fullmatch(r"0[xX][0-9A-Fa-f]{1,4}", token):
                raise ValueError(f"{path}:{lineno}: invalid u16")
            v = int(token, 16)
